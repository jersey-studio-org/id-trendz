...
function getTextureSrc(view) {
  return resolveAssetUrl(`/assets/${view}-track.png`, BASE_URL);
}
...
async function compositeTracksToImage(textureSrc, colorHex, svgEl) {
...
function TracksPanel({
  ...
  return (
    <div className="tracks-view-panel">
      <div className="tracks-view-badge">{VIEW_LABELS[view]} View</div>
      <div
      ...
      >
        ...
        <svg
          ref={svgRef}
          className="tracks-elements"
          viewBox={`0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_HEIGHT}`}
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          aria-label={`Tracks ${view} elements`}
          onPointerDown={(event) => {
            if (event.target === event.currentTarget) {
              onSelectElement?.(view, null);
            }
        ...
        >
          <defs>
            <filter id={`tracks-text-shadow-${view}`} x="-20%" y="-20%" width="140%" height="140%">
          ...
          {currentElements.map((el) => {
            ...
            if (el.type === 'text') {
              ...
              const pathId = curvePath ? `tracks-curve-${view}-${el.id}` : null;
...
const TracksTemplateCanvas = forwardRef(({
  colorHex = '#888888',
  viewSide = 'front',
    ...
    exportImage: async () => {
      ...
      try {
        const panelCanvases = await Promise.all(
          VIEW_ORDER.map((view) => compositeTracksToImage(
            getTextureSrc(view),
            colorHex,
        ...
        console.error('TracksTemplateCanvas: exportImage failed', error);
        return null;
      }
    },
  }));
  ...
  return (
    <div className={`tracks-template-views ${displayView === 'all' ? 'is-all-view' : ''}`}>
      <div className="tracks-active-stage">
        {VIEW_ORDER.map((view) => {
          const isVisible = displayView === 'all' || displayView === view;
          return (
            <div
              key={view}
              className={`tracks-view-stage ${isVisible ? 'is-active' : 'is-hidden'}`}
      ...
      <div className="jersey-view-dock" aria-label="Select tracks view">
        <button
          type="button"
...
TracksTemplateCanvas.displayName = 'TracksTemplateCanvas';
export default TracksTemplateCanvas;
