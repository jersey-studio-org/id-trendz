...
function getTextureSrc(view) {
  return resolveAssetUrl(`/assets/${view}-shorts.png`, BASE_URL);
}
...
async function compositeShortsToImage(textureSrc, colorHex, svgEl) {
  ...
  // Create color canvas: fill with colorHex, then clip to texture shape via destination-in
  ...
  colorCtx.drawImage(textureImg, 0, 0, exportWidth, exportHeight);

  // Draw texture first, then multiply-blend the color canvas on top, then elements
...
function ShortsPanel({
  ...
  return (
    <div className="shorts-view-panel">
      <div className="shorts-view-badge">{VIEW_LABELS[view]} View</div>
      <div
      ...
      >
        ...
        <svg
          ref={svgRef}
          className="shorts-elements"
          viewBox={`0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_HEIGHT}`}
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          aria-label={`Shorts ${view} elements`}
          onPointerDown={(event) => {
            if (event.target === event.currentTarget) {
              onSelectElement?.(view, null);
            }
        ...
        >
          <defs>
            <filter id={`shorts-text-shadow-${view}`} x="-20%" y="-20%" width="140%" height="140%">
          ...
          {currentElements.map((el) => {
            ...
            if (el.type === 'text') {
              ...
              const pathId = curvePath ? `shorts-curve-${view}-${el.id}` : null;
...
const ShortsTemplateCanvas = forwardRef(({
  colorHex = '#888888',
  viewSide = 'front',
    ...
    exportImage: async () => {
      ...
      try {
        const panelCanvases = await Promise.all(
          VIEW_ORDER.map((view) => compositeShortsToImage(
            getTextureSrc(view),
            colorHex,
        ...
        console.error('ShortsTemplateCanvas: exportImage failed', error);
        return null;
      }
    },
  }));
  ...
  return (
    <div className={`shorts-template-views ${displayView === 'all' ? 'is-all-view' : ''}`}>
      <div className="shorts-active-stage">
        {VIEW_ORDER.map((view) => {
          const isVisible = displayView === 'all' || displayView === view;
          return (
            <div
              key={view}
              className={`shorts-view-stage ${isVisible ? 'is-active' : 'is-hidden'}`}
      ...
      <div className="jersey-view-dock" aria-label="Select shorts view">
        <button
          type="button"
...
ShortsTemplateCanvas.displayName = 'ShortsTemplateCanvas';
export default ShortsTemplateCanvas;
